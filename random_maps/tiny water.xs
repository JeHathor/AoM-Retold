...
mutable void placeKotHObjects(int predatorProtoID = cUnitTypeShadePredator, bool initializeArea = true)
{
   // KotH override.
   if(gameIsKotH() == false)
   {
   ...
   int plentyID = rmObjectDefCreate("koth plenty vault");
   rmObjectDefAddItem(plentyID, cUnitTypePlentyVaultKOTH, 1);
   rmObjectDefPlaceAtLoc(plentyID, 0, cCenterLoc);
}
...
void generate()
{
   ...
   int waterTexture = cWaterTundraHybrid;
   ...
   else if(cNumberTeams == 2)
   {
      ...
      placePlayersOnLine(xFrac, 1.0 - zFrac, xFrac, zFrac);
      rmSetPlacementTeam(2);
      placePlayersOnLine(1.0 - xFrac, zFrac, 1.0 - xFrac, 1.0 - zFrac);
   }
   else
   {
   ...
   rmObjectDefPlacePerPlayer(startingCitadelCenterID, true, 1);
